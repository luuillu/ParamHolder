...
#define PH_PARAM_NAME_N(n) PH_REMOVE_FIRST_PARAM(PH_FOR_EACH_N(PH_PARAM_NAME_, n))

#define PH_FUNCTION_PARAM(i) , PH_TYPE_NAME(i) PH_PARAM_NAME(i)
#define PH_FUNCTION_PARAM_N(n)	PH_REMOVE_FIRST_PARAM(PH_FOR_EACH_N(PH_FUNCTION_PARAM, n))
...
#define PH_CONSTRUCT_INIT_PARAM(i) ,param_##i(param_##i)
#define PH_CONSTRUCT_INIT_PARAM_N(n) PH_GENERATE_COLON(n) PH_REMOVE_FIRST_PARAM(PH_FOR_EACH_N(PH_CONSTRUCT_INIT_PARAM, n))

#define PH_IS_MEMBER_FUNCION(...) __VA_ARGS__
#define PH_NOT_MEMBER_FUNCTION(...)

#define PH_PARAM_HOLDER_N(n, memberFunction)\
template<memberFunction(typename C,) typename RET PH_TEMPLATE_TYPE_NAME_N(n), template<typename> class Base>\
class ParamHolderImpl< RET (memberFunction(C::)*) (PH_TYPE_NAME_N(n)), Base>: public Base<ParamHolderImpl< RET (memberFunction(C::)*) (PH_TYPE_NAME_N(n)), Base> >\
{\
	typedef RET (memberFunction(C::)*FUNCTION_TYPE) (PH_TYPE_NAME_N(n));\
	typedef RET RETURN_TYPE;\
	memberFunction(typedef C CLASS_TYPE;)\
	typedef Base<ParamHolderImpl<FUNCTION_TYPE, Base> > BASE_TYPE;\
public:\
	ParamHolderImpl(PH_FUNCTION_PARAM_N(n)) PH_CONSTRUCT_INIT_PARAM_N(n) {}\
\
	RET invoke(memberFunction(C*pInstance,) FUNCTION_TYPE pFunc)\
	{\
		return (memberFunction(pInstance->*)pFunc)(PH_PARAM_NAME_N(n));\
	}\
\
private:\
...
PH_DEFINE_ALL(PH_PARAM_HOLDER_N, PH_IS_MEMBER_FUNCION)
PH_DEFINE_ALL(PH_PARAM_HOLDER_N, PH_NOT_MEMBER_FUNCTION)
